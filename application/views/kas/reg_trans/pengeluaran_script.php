...
<script>
    $('#form-1').ready(function () {
        ...
        $("select[name=id_kastrans_kategori]").select2({
            tags: true
        });
        ...
        $('#form-1').submit(function (e) {
            ...
            $.ajax({
                url: "<?= base_url() . $auth->get_url_controller() ?>submit_pengeluaran/", // Url to which the request is send
...
//                    console.log(data);
                    ...
                    if (!data.succes) {
                        ...
                        primary = data.data.primary_id;
//                        if (primary.trim().length > 0) {
//                            toastr.info(data.message, 'informasi');
//                        } else {
                            window.location.href = '<?= base_url() . $auth->get_url_controller() ?>detail/'+primary;
//                        }
                    } else {
